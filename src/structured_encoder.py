...
class StructuredEncoder:
    ...
    def set_params(self, params: dict = None, **kwargs) -> "StructuredEncoder":
        """
        Restore fitted parameters from a serialised dict or keyword arguments.

        Supports two calling conventions:
            enc.set_params({"mean_": ..., "std_": ...})   # dict
            enc.set_params(**{"mean_": ..., "std_": ...}) # kwargs (joblib / sklearn)
        """
        if params is not None:
            kwargs.update(params)
        if kwargs.get("mean_") is not None:
            v = kwargs["mean_"]
            self.mean_ = np.array(v) if not isinstance(v, np.ndarray) else v
        if kwargs.get("std_") is not None:
            v = kwargs["std_"]
            self.std_  = np.array(v) if not isinstance(v, np.ndarray) else v
        if "_fitted" in kwargs:
            self._fitted = kwargs["_fitted"]
        return self
